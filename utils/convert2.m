...
% *****************TEST END************************
    ...
    X = zeros(size(L{1},1)*size(L{1},2)*length(imagelist), size_L(3)); % # of pixels x 192 (scaled features)
    labels = zeros(size(L{1},1)*size(L{1},2)*length(imagelist), 1);
    ...
    for i = 1:length(imagelist)
        ...
        tmp = annotations(:, :, i); % 512*512
        % Save the labels
        labels((i-1)*voxels+1:i*voxels) = tmp(:);
%         for j = 1:size_L(3) %192
%             q = L{i}(:, :, j); % 512*512
%             X((i-1)*voxels:i*voxel, j) = q(:);
%         end
